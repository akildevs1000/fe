        ...
        <v-row class="px-4" style="">
          <v-col>
            <v-alert dense class="primary accent--text">
              Total Records:
              <strong v-if="orders.length > 0">{{ orders.length }}</strong>
            </v-alert>
          </v-col>
        </v-row>
        <v-toolbar flat>
                          ...
                          <template v-slot:default>
                            <thead>
                              <tr>
                                ...
                                <th class="text-left" v-if="!isReadOnly">
                                  Change Product
                                </th>
                                <th class="text-left" v-if="!isReadOnly">
                                  Change Quantity
                            ...
                            <tbody>
                              ...
                              >
                                ...
                                <td v-if="!isReadOnly">
                                  <v-text-field
                                    type="number"
                                    v-model="order_item_quantity[index]"
                                  ></v-text-field>
                                </td>
                                <td v-if="!isReadOnly">
                                  <v-btn
                                    ...
                                    @click="
                                      get_product(index, item.order_item_id)
                                    "
                                    fab
                                    x-small
                                    dark
                  ...
                  <v-form ref="form" lazy-validation>
                    <v-row>
                      ...
                      <v-simple-table dense>
                        <tbody>
                          <tr>
                            <th style="padding-left:75px; width:60%;">
                              Gross Amount
                            </th>
                            <td style="border:none;">
                              AED
                              {{ get_decimal_value(editedItem.order_gross) }}
                            </td>
                          </tr>
                          <tr>
                            <th style="padding-left:75px; width:60%;">
                            ...
                            <td style="border:;">
                              AED {{ get_decimal_value(editedItem.order_tax) }}
                            </td>
                          </tr>
                          <tr>
                            <th style="padding-left:75px; width:60%;">
                            ...
                            <th>
                              AED
                              {{ get_decimal_value(editedItem.order_total) }}
                            </th>
                          </tr>
                        </tbody>
                      </v-simple-table>
                    ...
                    <v-row>
                      <v-col>
                        <v-btn class="primary accent--text" text @click="close"
                          >Cancel</v-btn
                        >
                        &nbsp;
                        <v-btn
...
export default {
  ...
  methods: {
    ...
    searchIt() {
      this.$axios.get("search_order/" + this.search).then(res => {
        this.orders = res.data.orders;
        this.data = res.data;
      });
    },
    ...
    get_product(i, order_item_id) {
      ...
      this.$axios
        .post("change_order_item", payload)
        .then(res => {
          this.editedItem.order_gross = this.get_decimal_value(
            res.data.updated_record.order_gross
          );
          this.editedItem.order_tax = this.get_decimal_value(
            res.data.updated_record.order_tax
          );
          this.editedItem.order_total = this.orders[
            this.editedIndex
          ].order_total = this.get_decimal_value(
            res.data.updated_record.order_total
          );
          Object.assign(
            this.editedItem.products[i],
            res.data.updated_record.products
          );
    ...
    async paginate(e) {
      ...
      if (
        ...
        let data = await this.$axios.get("export_orders", {
          params: {
            sort_by: sortBy,
            order_by: orderBy
          }
    ...
    deleteItem(item) {
      confirm("Are you sure you want to delete this item?") &&
        this.$axios.delete("order/" + item.id).then(res => {
    ...
    save() {
      ...
      if (this.$refs.form.validate()) {
        ...
        this.$axios
          .post("status_change", payload)
          .then(res => {
            this.orders[this.editedIndex].status =
              res.data.updated_record.status;
