                  ...
                  <v-row>
                    <v-col>
                      <v-toolbar class="primary mb-2 accent--text" dark flat>
                        <v-toolbar-title
                          >Order # {{ editedItem.id }}</v-toolbar-title
                        >
                        <v-spacer></v-spacer>
                        <v-toolbar-title>{{
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td style="border:none;">
                              AED
                              {{ editedItem.order_gross | get_decimal_value }}
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td style="border:none;">
                              AED
                              {{
                                editedItem.discounted_price | get_decimal_value
                              }}
                            </td>
                          </tr>
                          <tr>
                            <th>Tax (VAT %5)</th>
                            <td style="border:;">
                              AED {{ editedItem.order_tax | get_decimal_value }}
                            </td>
                          </tr>
                          <tr>
                            <th>Grand Total</th>
                            <th>
                              AED
                              {{ editedItem.order_total | get_decimal_value }}
                            </th>
                          </tr>
                        </tbody>
                      </v-simple-table>
...
export default {
  ...
  methods: {
    async searchIt() {
      if (this.search) {
        let data = await this.$axios.get("search_order/" + this.search, {
          params: {
            per_page: 10,
            sort_by: "desc",
            order_by: "id"
        ...
        await this.$axios
          ...
          .then(res => {
            ...
            this.editedItem.order_total = this.orders[
              this.editedIndex
            ].order_total = res.data.updated_record.order_total;
            Object.assign(
    ...
    save() {
      if (this.$refs.form.validate()) {
        const error = this.editedItem.products.filter(v => {
          return (
            v.stock == 0 ||
            v.stock == "Stock does not exist" ||
            v.product_quantity > v.stock
          );
        });
